		...
		int handPos, int* bonus)
{
	...
	switch (card) {
	case adventurer: return playAdventurer(state);

	case council_room: return playCouncilRoom(state, handPos);

	case feast: return playFeast(state, choice1);

	case gardens:return -1;

	case mine:j = state->hand[currentPlayer][choice1];  //store card we will trash
	...
	case remodel:j = state->hand[currentPlayer][choice1];  //store card we will trash
	...
	case smithy: return playSmithy(state, handPos);

	case village: return playVillage(state, handPos);

	case baron: state->numBuys++;//Increase buys by 1!
	...
	case tribute:
		if ((state->discardCount[nextPlayer]+state->deckCount[nextPlayer])<=1) {
			if (state->deckCount[nextPlayer]>0) {
				...
				tributeRevealedCards[0] = state->discard[nextPlayer][state->discardCount[nextPlayer]
						-1];
	...
	case ambassador: j = 0;        //used to check if player has enough cards to discard
